...
def plot_sales(selected_route, distance_cost):
    ...
    ax.bar(len(store_names) + 1.5, total_cost, width=width * 2, color="grey", label="Total Costs (€)")

    ax.set_xticks([x + width for x in x_positions] + [len(store_names) + 1.5])
    ax.set_xticklabels(store_names + ["Total Costs"], rotation=45, ha="right")
