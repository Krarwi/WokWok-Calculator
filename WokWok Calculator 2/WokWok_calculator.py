...
icon_path = os.path.join(script_dir, r'WokWok.png')
...
def calculate_sales():
    ...
    totalsales = foodint * totalsalesamount

    messagebox.showinfo("Total Sales", f"The total sale amount is: {totalsales}€")
    ...
    ax.text(1.05, 0.90, f'Total Sales: {totalsales}€', transform=ax.transAxes, verticalalignment='top')
...
def import_data():
    ...
    if file_path:
        try:
            ...
            ax.text(1.05, 0.95, f'Total Sales: {total_sales}€', transform=ax.transAxes, verticalalignment='top')
...
food_options = ["Lohi Sushi", "Chicken and Rice", "Sushi"]
...
label_kmarket1.grid(row=2, column=0, padx=10, pady=10)
entry_kmarket1 = tk.Entry(root)
entry_kmarket1.grid(row=2, column=1, padx=10, pady=10)

label_kmarket2 = tk.Label(root, text="K-market Ryhimäkki Sales:")
label_kmarket2.grid(row=3, column=0, padx=10, pady=10)
entry_kmarket2 = tk.Entry(root)
entry_kmarket2.grid(row=3, column=1, padx=10, pady=10)

label_kmarket3 = tk.Label(root, text="K-market Kalasatama Sales:")
label_kmarket3.grid(row=4, column=0, padx=10, pady=10)
entry_kmarket3 = tk.Entry(root)
entry_kmarket3.grid(row=4, column=1, padx=10, pady=10)

btn_calculate = tk.Button(root, text="Calculate Total Sales", command=calculate_sales)
btn_calculate.grid(row=5, column=0, columnspan=2, padx=10, pady=20)

btn_export = tk.Button(root, text="Export Data", command=export_data)
btn_export.grid(row=6, column=0, columnspan=2, padx=10, pady=10)

btn_import = tk.Button(root, text="Import Data", command=import_data)
btn_import.grid(row=7, column=0, columnspan=2, padx=10, pady=10)
