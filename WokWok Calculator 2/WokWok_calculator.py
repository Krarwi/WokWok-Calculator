...
def update_store_options(selected_round):
    new_stores = round_store_mapping.get(selected_round, round_store_mapping["Default"])

    selected_store_var.set(new_stores[0])
    dropdown_store['menu'].delete(0, 'end')

    for store in new_stores:
        dropdown_store['menu'].add_command(
            label=store, 
            command=tk._setit(selected_store_var, store, lambda store=store: set_store_price(store))
        )
...
def import_data():
    ...
    if file_path:
        ...
        for row, (food_item, info) in enumerate(all_sales_data.items(), start=5):
            ...
            root.nametowidget(f"entry_{food_item}_amount").insert(0, amount)

            sale_rate = info.get("sale_rate", "20%")
            sales_inputs[food_item]["sale_rate_var"].set(sale_rate)
...
label_food = tk.Label(root, text="Select Food Type:", foreground="snow", font=("Helvetica", 9, "bold"))
label_food.grid(row=0, column=0, padx=10, pady=10)
label_food.configure(background="gray25")
...
dropdown_food.configure(background="gray30", font=("Times", 9, "bold"))

label_store = tk.Label(root, text="Select Store:", foreground="snow", font=("Helvetica", 9, "bold"))
label_store.grid(row=1, column=0, padx=15, pady=10)
label_store.configure(background="gray23")
...
dropdown_store.configure(background="gray35", font=("Times", 9, "bold"))

label_date = tk.Label(root, text="Select Date:", foreground="snow", font=("Helvetica", 9, "bold"))
label_date.grid(row=2, column=0, padx=10, pady=10)
label_date.configure(background="gray20")
date_entry = DateEntry(root, width=12, background='darkblue', foreground='white', borderwidth=2)
date_entry.grid(row=2, column=1, padx=10, pady=10)
date_entry.configure(font=("Times", 9, "bold"))

label_route = tk.Label(root, text="Select Round:", foreground="snow", font=("Helvetica", 9, "bold"))
label_route.grid(row=3, column=0, padx=10, pady=10)
label_route.configure(background="gray17")
routes = ["Round 1", "Round 2", "Round 3", "Round 4", "Round 5", "Round 6", "Round 7", "Round 8", "Overall"]
selected_route_var = tk.StringVar(root)
selected_route_var.set(routes[0])
selected_route_var.trace("w", lambda *args: update_store_options(selected_route_var.get()))
dropdown_route = tk.OptionMenu(root, selected_route_var, *routes)
dropdown_route.grid(row=3, column=1, padx=10, pady=10)
dropdown_route.configure(background="gray40", font=("Times", 9, "bold"))

btn_calculate = tk.Button(root, text="Calculate Total Sales", command=calculate_sales, font=("Times", 9, "bold"))
btn_calculate.grid(row=4, column=0, padx=10, pady=10)

btn_export = tk.Button(root, text="Export Sales Data", command=export_data, font=("Times", 9, "bold"))
btn_export.grid(row=4, column=1, padx=10, pady=10)

btn_import = tk.Button(root, text="Import Sales Data", command=import_data, font=("Times", 9, "bold"))
btn_import.grid(row=4, column=2, padx=10, pady=10)

btn_reset = tk.Button(root, text="Reset Sales Data", command=reset_sales_data, font=("Times", 9, "bold"))
btn_reset.grid(row=4, column=3, padx=10, pady=10)
